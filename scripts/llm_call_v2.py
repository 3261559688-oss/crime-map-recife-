...
def get_pending_items(conn, stage, limit, state=None, city=None):
    ...
    if stage == 'a':
        sql = f"SELECT event_id,title,description,source_media FROM dwd_intl_crime_incident_di WHERE llm_a_is_crime IS NULL{sf}{cf}"
    elif stage == 'b':
        sql = f"SELECT event_id,title,description,crime_type FROM dwd_intl_crime_incident_di WHERE llm_a_is_crime=1 AND llm_b_type IS NULL{sf}{cf}"
    elif stage == 'c':
        sql = f"SELECT event_id,title,description,news_url,state,city,city_method FROM dwd_intl_crime_incident_di WHERE llm_a_is_crime=1 AND llm_c_state IS NULL{sf}{cf}"
